...
// ReadConfigFromDir reads config directly from the assets directory.
func ReadConfigFromDir(assetsDir string) (Config, error) {
	return ReadConfig(assetsDir, "")
}
...
func configPath(assetDir, name string) string {
	if name == "" || name == standardModelName {
		return path.Join(assetDir, configFile)
	}
...
func modelPath(assetDir, name string) string {
	if name == "" || name == standardModelName {
		return path.Join(assetDir, modelFile)
	}
