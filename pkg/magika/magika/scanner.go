...
func NewScanner(assetsDir, name string) (*Scanner, error) {
	...
	if ckb == nil {
		return nil, errors.New("read content types KB: nil ckb")
	}
	return &Scanner{
		onnx: ob,
		cfg:  cfg,
...
// NewScannerFromConfig returns a scanner using the given config and content types KB.
func NewScannerFromConfig(cfg Config, ckb map[string]ContentType, modelPath string) (*Scanner, error) {
	ob, err := onnx.NewOnnx(modelPath, len(cfg.TargetLabelsSpace))
	if err != nil {
		return nil, fmt.Errorf("new onnx: %w", err)
	}
	if ob == nil {
...
func (s *Scanner) Scan(r io.ReaderAt, size int) (ContentType, error) {
	if s == nil {
		return ContentType{}, errors.New("scan: nil scanner")
	}
	if size == 0 {
	...
	if s.onnx == nil {
		return ContentType{}, errors.New("scan: nil onnx")
	}
