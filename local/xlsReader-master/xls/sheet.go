...
func (rw *rw) GetCols() (cols []structure.CellData) {
	...
	cols = make([]structure.CellData, maxColKey+1)
	for i := 0; i <= maxColKey; i++ {
		if rw.cols[i] == nil {
			cols[i] = new(record.FakeBlank)
		} else {
			cols[i] = rw.cols[i]
		}
	}
...
func (s *Sheet) GetRows() (rows []*rw) {
	...
	numRows := s.GetNumberRows()
	rows = make([]*rw, numRows)
	for i := 0; i < numRows; i++ {
		if s.rows[i] == nil {
			r := new(rw)
			r.cols = make(map[int]structure.CellData)
...
func (s *Sheet) addCell(cd structure.CellData, row [2]byte, column [2]byte) {
	...
	rowData, rowExists := s.rows[r]
	if !rowExists {
		rowData = new(rw)
		rowData.cols = make(map[int]structure.CellData)
		s.rows[r] = rowData
	}
